...
require_once("../librerias/lib/connection.php");
		...
		//('SuperQueryTurnos @ano=2014, @mes=1 , @usuario=JSABOGAL, @cargo=503205, @centrocosto=60040203,  @param=0')"
...
$rs=$conn->Execute($sql);
		while($row10 = $rs->fetchrow()){
			...
			for($i=0; $i<$cont; $i++){
			    ...
			    $table.="<tr class='si' id='".($i+1)."'>";
				
				$table.="<td align='center'>".($i+1)."</td><td  class='context-menu-one empleado ".$empleado." ".@$lista0[$i]['cod_epl']."' id='".@$lista0[$i]['id']."'style='cursor:pointer;'  align='left'>".@$lista0[$i]['empleado']."</td>";
				...
				for($j=1; $j <= $cantidad; $j++){
				   ...
				   /* $sql1="select * from feriados where fec_fer='$anio-$j-$mes'";
					...
					$table .= "<td align='center' class='context-menu-two ".$empleado." ".@$lista0[$i]['cod_epl']."' >".@$lista0[$i][$j]."</td>";	
