...
if($_POST['modulo']==1){ //para vacaciones en disfrute
	...
	$sql2="insert into bs_ausencias_vac(cod_con,cod_epl,fec_ini,fec_fin,estado,cod_cc2,dias,cod_aus,fec_ini_r,fec_fin_r,cnsctvo,dias_pen,fecha)
...
function contar_semana($id,$mes,$anio){
	...
	while($row=$rs4->fetchrow()){ // inicio del while
		...
		for($i=1;$i<=31;$i++){ //inicio  del for
			...
			if($turno !='V' AND $turno!='X'  AND $turno!='N' AND $turno!='LM'){
				...
				if($rs5->RecordCount() > 0){
					...
					$sql6="select horas from turnos_prog_tmp where cod_tur='".$turno."'";
