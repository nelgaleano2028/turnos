...
function main(){
	...
	//nuevo
	$('.tabla #predeter input').keyup(filtros);		
	//fin nuevo
...
function imprimir_contenido(){
	  ...
	  window.open("imprimir_turno_pre.php", '_blank');
...
var cuanto=$(".codigo_turno").length;
	...
	var	mensaje = '<div id="usuario_n" style="color: rgb(0,0,0) !important; text-align: center;">';
		...
		mensaje += "<input type='hidden' name='info' value='3' />";
...
function modaleditar_turno(){
	...
	var	mensaje = '<div id="usuario_n" style="color: rgb(0,0,0) !important; text-align: center;">';
		...
		mensaje += "<div  style='display:inline-block; width:30%'><label >Horario Inicial</label>";
		mensaje += "<select style='width:80%;' class='fecha_ini' name='fecha_ini'><option value='"+horas_ini_turno[1]+"'>"+horas_ini_turno+"</option>";
		for(var i=0;i<24;i++){
		mensaje+="<option value='"+i+"'>"+i+":00</option>";
		}
		mensaje += "</select></div>";
		mensaje += "<div  style='display:inline-block; width:30%'><label >Horario Final</label>";
		mensaje += "<select style='width:80%;' class='fecha_fin' name='fecha_fin'><option value='"+horas_fin_turno[1]+"'>"+horas_fin_turno+"</option>";
		for(var i=0;i<24;i++){
		mensaje+="<option value='"+i+"'>"+i+":00</option>";
		}
		mensaje += "</select></div>";
		...
		mensaje += "<input type='text' class='horas' readonly='readonly' name='horas' value='"+horas_turno+"' style='width: 40%;'/></div>";
		mensaje += "<input type='hidden' name='codigo_old' value='"+codigo_turno+"' style='width:40%;'/>";
		mensaje += "<input type='hidden' name='info' value='2' />";
		mensaje += "<div class='validar'style='color:#fff !important; text-align:center;'></div>";
...
function modalcrear_turno(){
	...
	var	mensaje = '<div id="usuario_n" style="color: rgb(0,0,0); text-align: center;">';
		...
		mensaje += "<div style='display:inline-block; width:20%' ><label >C&oacute;digo</label>";
		mensaje += "<input type='text' maxlength='3' class='codigo' onkeyup='this.value=this.value.toUpperCase();' name='codigo' style='width: 40%;'/></div>";
		mensaje += "<div  style='display:inline-block; width:30%'><label >Horario Inicial</label>";
		...
		mensaje += "<input type='hidden' name='info' value='1' /></div>";
		mensaje += "<div class='validar' style='color:#fff !important; margin-left:30%;'></div>";
		mensaje += "<div style='margin-top:5%;'><input id='crear_turno' type='button' class='btn btn-primary' style='margin-left:38%'  value='Crear Turno'/></div>";
		mensaje += "</form>";
		mensaje += "</div>";
		mensaje += "</div>";
...
function crear_turno(){
	...
	if($('#nuevo_turno .codigo').val()== 0 || $('.fecha_ini').val()== -1 || $('.fecha_fin').val()==-1){
...
function filtros(){
	...
	if(recibe.length>0 && detector=='codigo'){
        ...
        $("#predeter tr td.codigo").not(":Contains('"+recibe+"')").parent().hide();
		
	}
	if(recibe.length>0 && detector=='hora'){
		 // con la clase .nombre le decimos en cual de las celdas buscar y si no coincide, ocultamos el tr que contiene a esa celda. 
        $("#predeter tr td.hora").not(":Contains('"+recibe+"')").parent().hide();
	}
